...
{
	return;		// Debug, the emission will be handled inside the CPU
	// Spawn Interval
	if (input.Timer <= 0)
	{
		// Spawn Loop
		for (uint i = 0; i < g_emitterRate; i++)
		{
			...
			//reset our timer
			input.Timer = g_fSecondsPerParticle /* + vRandom.x*0.4*/;
		}
	}
	else
	{
...
{
	// Emitter Particle
	if (input[0].Type == PT_LAUNCHER)// EMMITER PARTCILE
		GSLauncherHandler(input[0], ParticleOutputStream);
	else if (input[0].Type == PT_SHELL)// NORMAL PARTICLE
